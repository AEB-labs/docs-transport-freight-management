  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        The correct SSL certificate list must still be entered in the connection.\
    ...
    <tr>
      <td>
        'ICM\_CONNECTION\_FAILED' and in log file dev.icm the error 'NIECONN\_REFUSED' occurs.
      </td>

      <td>
        The name resolution for the host address "rz.aeb.de" does not work because DNS is not set correctly.\
    ...
    <tr>
      <td>
        The log file dev.icm contains:\
        the verification of the server's certif-icate chain failed' or also\
        Untrusted Chain PEER\_CERT\_UNTRUSTED
      </td>

      <td>
        The SSL certificates have not been read correctly, or not all of them are known in the PSE. This error is also described in more detail in SAP Note 1094342.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Error pattern: There are repeated interruptions. In between, however, many calls also work. The ICM log contains the error message NIECONN\_REFUSED for the affected user, but with the IP address of the proxy server. This user has no authorization to establish an external connection via the proxy.
      </td>
    </tr>
