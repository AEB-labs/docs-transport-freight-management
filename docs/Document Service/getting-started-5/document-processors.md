...
The document preparation consists of merging of a predefined <Glossary>Document template</Glossary>s, available in the *Document Service*, with customer data, transmitted in a customer request. The following <Glossary>Document processor</Glossary>s are available:

## PDF-XFA

This processor handles PDF Forms based on the XFA standard. Flat PDF files are generated from the template form. However, only a limited set of XFA features and a couple of non-standard extension is supported. Therefore it is crucial that the templates forms are prepared by experts from the *AEB document community*.
...
[Get <Glossary>Document processor tag</Glossary>s](/reference/getsupportedprocessors) available in your context.
...
* One of the `supportedFormats` values can be used to specify the desired <Glossary>Document content</Glossary> output format when a document is prepared.
* The `supportedParseCommands` can be used to extract derived or additional content from the <Glossary>Document template</Glossary>, like the data structure or example documents.
