...
[block:parameters]
{
  "data": {
    ...
    "13-1": "Error pattern: There are repeated interruptions. In between, however, many calls also work. The ICM log contains the error message NIECONN_REFUSED for the affected user, but with the IP address of the proxy server. This user has no authorization to establish an external connection via the proxy.",
